...
// DeepSeek Harness (DSH) Client UI plugin (ModuleLoader format).
//
// This plugin is designed to be loaded by DSH in the browser.
// It injects CSS tokens, sets favicon, and enforces the document title.
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx, config) {
      ...
      // 1) Inject CSS (reversible)
      if (typeof document !== "undefined") {
        ctx.effect(() => {
          const tag = document.createElement("style");
          tag.dataset.plugin = "@aibizhive/dsh-client-ui-aibizhive-theme";
          tag.dataset.pluginCss = "@aibizhive/dsh-client-ui-aibizhive-theme/skin.css";
      ...
      if (typeof document !== "undefined" && cfg.title) {
        ctx.effect(() => {
          ...
          const t = setInterval(
            enforce,
            Math.max(50, Number(cfg.enforceIntervalMs || 250))
          );
          return () => {
            try {
